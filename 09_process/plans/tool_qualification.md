...
| `tools/jobs.py` | Captures tasks written by the human in the requirements browser and runs them through the Claude Code CLI | no | Produces no work product of its own, but it is the one tool here that **runs an agent against the repository**, so its failure mode is not a wrong artefact but an unintended change. Three properties carry the argument: `run` always prints the job and the exact command and waits for confirmation, work happens on its own branch and is never committed or pushed, and the companion server binds to loopback and never accepts a path from the client. A job file is data that becomes an instruction — one arriving from someone else is read before it is run, and jobs are never run in CI. |
